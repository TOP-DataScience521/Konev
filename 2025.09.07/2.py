# я решил взять геренальную совокупность из температур своего города, Великий Новгород за 2023 год за каждый день
# в этом файле буду работать с нормироваными высотами
...
# попоробую разделить на 4 интервала аналогично температурным сезонам, возьму максимум 40 и минимум -30 в модуле,сложу и разделю на 4 
# тем самым примерно определю интервалы
X_intervals_4 = [(-30, -13), (-13,4), (4, 21), (21, 40)]
m_intervals_4 = []

# Расчет частот для интервалов (как в stat1.py)
...
h_intervals_4 = [m_intervals_4[i] / X_intervals_widths_4[i] for i in range(len(X_intervals_4))]

# тут разделю совокупность на 12 интервалов как будто на 12 температурных месяцев, 
# так же диапазон температур от 40 до -30 по модулю разделю на 12 более менее равных отрезков
...
h_intervals_12 = [m_intervals_12[i] / X_intervals_widths_12[i] for i in range(len(X_intervals_12))]

# построим три графика с нормировкой
fig = plt.figure(figsize=(10, 5), dpi=100)
axs = fig.subplots(1, 3)

#   4 интервала с нормировкой 
axs[0].bar(
    x=X_intervals_means_4,
    height=h_intervals_4,
    width=X_intervals_widths_4,
)
axs[0].set_title('4 интервала (сезоны)')

#  график 12 интервалов с нормировкой 
axs[1].bar(
    x=X_intervals_means_12,
    height=h_intervals_12,
    width=X_intervals_widths_12,
)
axs[1].set_title('12 интервалов (месяцы)')

# столбчатая диаграмма дискретного ряда , визуально лучше анализируемая
...
# пробую построить автоматические графики
fig2 = plt.figure(figsize=(10, 5), dpi=100)
axs2 = fig2.subplots(1, 2)

axs2[0].hist(x=sample, bins=4)
axs2[0].set_title('авто 4 интервала')

axs2[1].hist(x=sample, bins=12)
