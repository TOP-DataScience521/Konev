...
"""
    ...
    - Среднее гармоническое (harmonic)

    параметры:
        num_1 (float): Первое обязательное число 
        num_2 (float): Второе обязательное число 
        *num_x (float): Произвольное количество дополнительных чисел

    returns:
        dict[str, float]: Словарь с вычисленными мерами центральной тенденции:
            {
                'median': float,    # Медиана
                'arithmetic': float, # Среднее арифметическое
                'geometric': float, # Среднее геометрическое
                'harmonic': float    # Среднее гармоническое
            }
"""
